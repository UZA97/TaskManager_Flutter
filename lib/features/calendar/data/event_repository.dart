...
class EventRepository {
  ...
  Future<List<Event>> getEventsByMonth(int year, int month) async {
    ...
    final rows = await (_db.select(_db.eventTable)
  ...
  Future<void> addEvent(Event event) async {
    await _db.into(_db.eventTable).insert(
  ...
  Future<void> updateCompletion(int id, bool isCompleted) async {
    await (_db.update(_db.eventTable)..where((t) => t.id.equals(id)))
        .write(EventTableCompanion(isCompleted: Value(isCompleted)));
  }
